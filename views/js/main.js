...
/* With the help of this function 
   we can Capitalizes first letter of each word*/

/* Name generator extracted 
...
// fetch adjective from array using random number sent from generator
function get_adj(x) {
    switch (x) {
        case "dark":
...
// fetch noun out of array using random number sent from generator
function get_noun(y) {
    switch (y) {
        case "animals":
...
/* different types of adjectives for pizza titles*/

let adjectives = ["dark", "color", "whimsical", "shiny", "noisy", "apocalyptic", "insulting", "praise", "scientific"];
/* types of nouns for pizza titles*/
let nouns = ["animals", "everyday", "fantasy", "gross", "horror", "jewelry", "places", "scifi"];

/*Generates random numbers for get_adj and 
get_noun functions and returns a new pizza name*/
function generator(adj, noun) {
    let adjectives = get_adj(adj);
    let nouns = get_noun(noun);
...
/* Select Random adjective and random noun */
function randomName() {
...
/* This functions return a string of a random 
...
let pizzaElementGenerator = function (i) {
    let pizzaContainer, // contain pizza title, image and list of ingredients
        pizzaImageContainer, // contain image of pizza
        pizzaImage, //  pizza image
        pizzaDescriptionContainer, // contain the  title and list of ingredients of pizza
        pizzaName, // name of pizza
    ...
    pizzaContainer.id = "pizza" + i; // every pizza element have a unique id by this addition
...
// when 'Our Pizzas Section moves the slider inside the section it calls the resizePizzas(size) function
let resizePizzas = function (size) {
    window.performance.mark("mark_start_resize"); // User Timing API function

    // Changes the value of the size for the pizza above the slider
    ...
    // Gives the size variations to change a pizza element from one size to different and this is called by changePizzaSlices(size).
    function determineDx(elem, size) {
        ...
        // Converts  the slider value to a percentage width
    ...
    // triverse from pizza elements on the webpage and changes their widths
    function changePizzaSizes(size) {
        let pizzaContainerLength = document.getElementsByClassName("randomPizzaContainer").length;
        // setting of randomPizzaContainer
        let randomPizzaContainer = document.getElementsByClassName("randomPizzaContainer");
        let newwidth = [];
        for (let n = 0; n < pizzaContainerLength; n++) {
            let dx = determineDx(randomPizzaContainer[n], size);
            newwidth[n] = (randomPizzaContainer[n].offsetWidth + dx) + 'px';
        }
        // Created a new for loop to set the styles of the containers prevent the browser to paint so much. 
        for (let cl = 0; cl < pizzaContainerLength; cl++) {
            randomPizzaContainer[cl].style.width = newwidth[cl];
        }
    }
...
for (let ac = 2; ac < 100; ac++) {

    pizzasDiv.appendChild(pizzaElementGenerator(ac));
}
...
/* The following code for sliding background pizzas was fetched from Ilya's demo found at:
   https://www.igvita.com/slides/2012/devtools-tips-and-tricks/jank-demo.html*/

/* Move the sliding background pizzas on the basis of scroll position*/
function updatePositions() {
    frame++;

    let items = document.getElementsByClassName('mover');
    ...
    let stuffy = document.body.scrollTop / 1250;
    for (let m = 0; m < items.length; m++) {
        let phase = Math.sin((stuffy) + (m % 5));
        items[m].style.left = items[m].basicLeft + 100 * phase + 'px';
    }
...
document.addEventListener('DOMContentLoaded', function () {
    let cols = 8;
    let s = 256;
    let pizzasToDisplay = Math.ceil(screen.height / s) * 8;
    console.log(pizzasToDisplay);
    let movingPizzas = document.getElementById('movingPizzas1');
    
    for (let i = 0; i < pizzasToDisplay; i++) {
    ...
    /* On Page Load it moves the items variable out of the loop above. It's initialized in the
     * Global Scope, and then assigned a value once */
